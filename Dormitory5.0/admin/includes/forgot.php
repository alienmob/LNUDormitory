<?php
 ...
 if(ISSET($_POST['forgot'])){
  ...
  if(!$email){
   $_SESSION['email_error'] = 'Error! Please Input a Valid Email.';
  }
  else
  {
   ...
   if($query->num_rows < 1){
    $_SESSION['email_error'] = 'Email Does Not Exist in the Database.';
     ...
     if ($conn->query($sql_insert)) {
      ...
      $output.='<p><a href="http://localhost/Dormitory5.0/admin/reset-password.php?token='.$token.'&email='.$email.'&action=reset" target="_blank">
      http://localhost/Dormitory5.0/admin/reset-password.php
      ...
      try {
       ...
       $_SESSION['reset_success'] = 'Reset Password link sent! Please Check your Email.';
       
      } catch (Exception $e) {
       echo "Message could not be sent. Mailer Error: {$mail->ErrorInfo}";
       $_SESSION['not_sent'] = 'An error occured. Message was not sent! Check Your Internet Connection & Try Again.';
      }
     }
     else {
